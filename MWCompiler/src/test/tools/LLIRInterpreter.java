...
/**
 * Powered by abcdabcd987 on 2016-04-23.
 *
 * Modified by Micheal Wu on 2018-05-19.
 */
public class LLIRInterpreter {
    ...
    static private final Set<String> opnames1 = new HashSet<>(Arrays.asList("store", "br", "jmp", "ret", "call"));
    ...
    static private final Set<String> opnum1 = new HashSet<>(Arrays.asList("ret", "jmp", "MOV", "NEG", "NOT", "alloc"));
    ...
    private void readInstruction() throws SemanticError {
        ...
        if (line.startsWith("%")) {
            if (!line.endsWith(":"))
                throw new SemanticError("expected a `:`");
        ...
        if (split.length == 1) {
            if (!opnames1.contains(inst.operator))
                throw new SemanticError("illegal operator " + inst.operator);
        } else if (split.length == 2) {
            if (!opnames2.contains(inst.operator))
                throw new SemanticError("illegal operator " + inst.operator);
        } else {
            throw new SemanticError("illegal operator " + inst.operator);
        }
        ...
        switch (inst.operator) {
            ...
            case "phi":
                if (!allowPhi)
                    throw new SemanticError("`phi` is not allowed here");
                if ((words.size() & 1) == 0)
                    throw new SemanticError("the number of `phi` argument should be even");
                ...
                for (int i = 1; i < words.size(); i += 2) {
                    ...
                    if (!label.startsWith("%"))
                        throw new SemanticError("label should starts with `%`");
            ...
            default:
                if (split.length == 2)
                    inst.dest = split[0].trim();
                inst.op1 = words.get(1);
                if (opnum1.contains(inst.operator))
                    return;
    ...
    private final Set<String> opjump = new HashSet<>(Arrays.asList("br", "jmp", "ret"));

    private int memoryRead(int addr) throws RuntimeError {
        Byte data = memory.get(addr);
        if (data == null)
            throw new RuntimeError("memory read violation");
        return data & 0xFF;
    }
    ...
    private int registerRead(String name) throws RuntimeError {
        ...
        if (reg == null)
            throw new RuntimeError("register `" + name + "` haven't been defined yet");
        return reg.value;
    }

    private void registerWrite(String name, Integer value) throws RuntimeError {
        if (!name.startsWith("$"))
            throw new RuntimeError("not a register");
        Register reg = registers.get(name);
        if (reg == null) {
            reg = new Register();
    ...
    private void runInstruction() throws RuntimeError {
        if (++cntInst >= instLimit)
            throw new RuntimeError("instruction limit exceeded");
        switch (curInst.operator) {
            ...
            case "call":
                ...
                if (curInst.dest != null)
                    registerWrite(curInst.dest, retValue);
                return;
            ...
            if (!curBB.phi.isEmpty()) {
                ...
                for (PhiNode phi : curBB.phi) {
                    ...
                    if (regName == null) {
                        ...
                        int value = regName.equals("undef") ? (int) (Math.random() * Integer.MAX_VALUE)
                                : readSrc(regName);
